...
// Simplified global long/short ratio data
...
async function execute(input: ReaderInput): Promise<ReaderOutput> {
  ...
  try {
    ...
    console.log(
      `[Reader] Fetching global LS account ratio for ${symbol} ${period}, limit: ${limit}`
    );

    // Build Binance perpetual futures API request
    const baseUrl = 'https://fapi.binance.com/futures/data/globalLongShortAccountRatio';
